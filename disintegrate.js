...
// starts ONLY after ".click-to-enter" is clicked
// per-person: no repeats until all songs have played (uses localStorage)
...
(function () {
  ...
  if (!audio || !volumeSlider || !SONGS.length) return;
  ...
  const KEY_ORDER = "music_order_v1";
  const KEY_INDEX = "music_index_v1";

  function getOrder() {
    const saved = localStorage.getItem(KEY_ORDER);
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        if (Array.isArray(parsed) && parsed.length === SONGS.length) return parsed;
      } catch {}
    }
    const newOrder = shuffle(SONGS);
  ...
  function getIndex() {
    const n = parseInt(localStorage.getItem(KEY_INDEX) || "0", 10);
  ...
  function pickNextSong() {
    ...
    const song = order[idx];
    setIndex(idx + 1);
    return song;
  }

  function startMusic() {
    audio.src = pickNextSong();
    audio.loop = false;
  ...
  // start ONLY after click-to-enter
  const clickToEnter = document.querySelector(".click-to-enter");
  if (clickToEnter) {
    clickToEnter.addEventListener("click", () => startMusic(), { once: true });
  }
})();
